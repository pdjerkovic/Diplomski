...
from dodaci import modcrop_color, ucitaj_ckpt, sacuvaj_ckpt, SRCNN
import numpy as np
import os
import time
import cv2 as cv
...
def preprocesiranje(dirpath, config):
	...
	for i in datai:
		...
		print("H: ", h, " W: ", w, "im_input.shape ", im_input.shape)
		...
		for images in range(0, h - config.i_size, config.stride):
			for y in range(0, w - config.i_size, config.stride):
				...
				subim_input = subim_input.reshape([config.i_size, config.i_size, 3])
				subim_label = subim_label.reshape([config.l_size, config.l_size, 3])
	...
	print("Data ima oblik ", data.shape)
	print("GT ima oblik ", label.shape)
...
def trening(img_dir, config):
	...
	images = tf.placeholder(tf.float32, [None, config.i_size, config.i_size, 3], name='images')
	labels = tf.placeholder(tf.float32, [None, config.l_size, config.l_size, 3], name='labels')
